...
const settingsCards = [
    ...
    ['Theme', 'Dark-only interface for portfolio consistency.', 'Ready'],
...
const statusClass: Record<string, string> = {
    ...
    Planning: 'bg-sky-500/15 text-sky-300 ring-sky-500/30',
    Planned: 'bg-sky-500/15 text-sky-300 ring-sky-500/30',
    Backlog: 'bg-slate-700 text-slate-200 ring-slate-600',
    Invited: 'bg-violet-500/15 text-violet-300 ring-violet-500/30',
    ...
    return (
        <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 mb-6">
            {items.map((metric) => {
                ...
                return (
                    <article key={metric.label} className="rounded-2xl sm:rounded-3xl bg-slate-900 p-4 sm:p-5 shadow-sm border border-slate-800">
                        <div className="flex items-center justify-between gap-3 mb-5">
                            <div className="h-11 w-11 sm:h-12 sm:w-12 rounded-2xl bg-indigo-500/15 text-indigo-300 flex items-center justify-center text-xl sm:text-2xl shrink-0">
                                <Icon />
                            </div>
                            <span className="rounded-full bg-emerald-500/15 px-2.5 sm:px-3 py-1 text-[11px] sm:text-xs font-bold text-emerald-300 whitespace-nowrap">
                                {metric.change}
                            </span>
                        </div>
                        <div className="text-xs sm:text-sm text-slate-400">{metric.label}</div>
                        <div className="text-2xl sm:text-3xl font-bold mt-1 text-white">{metric.value}</div>
                    </article>
                )
            })}
        </section>
...
function DemoTable({
    ...
    return (
        <section className="rounded-2xl sm:rounded-3xl bg-slate-900 p-4 sm:p-6 shadow-sm border border-slate-800">
            <div className="mb-4">
            ...
            <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-left text-sm min-w-[640px]">
                    ...
                    <tbody>
                        {rows.map((row) => (
                            <tr key={row.join('-')} className="border-t border-slate-800">
                                {row.map((cell, index) => (
                                    <td key={`${cell}-${index}`} className="py-4 pr-4">
                                        {index === row.length - 1 ? (
                                            <StatusBadge value={cell} />
                                        ) : index === 0 ? (
                                            <span className="font-semibold text-indigo-300">{cell}</span>
                                        ) : (
            ...
            <div className="md:hidden space-y-3">
                {rows.map((row) => (
                    <article key={row.join('-')} className="rounded-2xl border border-slate-800 bg-slate-950 p-4">
                        <div className="flex items-start justify-between gap-3 mb-3">
                            <div>
                                <div className="text-xs uppercase tracking-wide text-slate-500">
                                    {headers[0]}
                                </div>
                                <div className="font-semibold text-indigo-300 break-words">
                                    {row[0]}
                                </div>
                            </div>
                        ...
                        <div className="grid gap-2 text-sm">
                            {row.slice(1, -1).map((cell, index) => (
                                <div key={`${cell}-${index}`} className="flex flex-col gap-1">
                                    ...
                                    <span className="text-slate-300 break-words">{cell}</span>
                                </div>
                            ))}
                        </div>
                    </article>
...
function ProgressList({ items }: { items: ProgressItem[] }) {
    return (
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {items.map((item) => (
                <article key={item.label} className="rounded-2xl sm:rounded-3xl bg-slate-900 p-4 sm:p-6 shadow-sm border border-slate-800">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-5">
                        <div>
                            <h2 className="text-lg sm:text-xl font-bold text-white">{item.label}</h2>
                    ...
                    <div className="mt-3 text-sm font-semibold text-slate-300">{item.progress}% complete</div>
                </article>
            ))}
        </section>
    )
    ...
    return (
        <>
            ...
            <section className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
                <article className="xl:col-span-2 rounded-2xl sm:rounded-3xl bg-slate-900 p-4 sm:p-6 shadow-sm border border-slate-800">
                    <div className="flex items-start justify-between gap-4 mb-6 sm:mb-8">
                        <div>
                            <h2 className="text-lg sm:text-xl font-bold text-white">Workspace activity</h2>
                            <p className="text-sm text-slate-400">Mock activity trend for the current month.</p>
                        </div>
                        <PiChartLineUpDuotone className="text-2xl sm:text-3xl text-indigo-300 shrink-0" />
                    </div>
                    <div className="flex items-end gap-2 sm:gap-3 h-48 sm:h-64 lg:h-72">
                        {chartBars.map((height, index) => (
                            <div key={index} className="flex-1 rounded-t-xl sm:rounded-t-2xl bg-indigo-500/10 relative overflow-hidden min-w-0">
                                <div className="absolute bottom-0 left-0 right-0 rounded-t-xl sm:rounded-t-2xl bg-indigo-500" style={{ height: `${height}%` }} />
                            </div>
                        ))}
                    </div>
                </article>
                <article className="rounded-2xl sm:rounded-3xl bg-slate-900 p-4 sm:p-6 shadow-sm border border-slate-800">
                    <h2 className="text-lg sm:text-xl font-bold mb-2 text-white">Goal progress</h2>
                    <p className="text-sm text-slate-400 mb-6">Operational completion rate.</p>
                    <div className="h-36 w-36 sm:h-44 sm:w-44 rounded-full border-[14px] sm:border-[18px] border-indigo-400 border-r-slate-800 mx-auto flex items-center justify-center">
                        <div className="text-center">
                            <div className="text-3xl sm:text-4xl font-bold text-white">76%</div>
        ...
        <section className="space-y-4 sm:space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                {reportCards.map(([title, value, description, trend]) => (
                    <article key={title} className="rounded-2xl sm:rounded-3xl bg-slate-900 p-4 sm:p-6 shadow-sm border border-slate-800">
                        <div className="h-11 w-11 sm:h-12 sm:w-12 rounded-2xl bg-indigo-500/15 text-indigo-300 flex items-center justify-center text-xl sm:text-2xl mb-5">
                            <PiChartLineUpDuotone />
                        </div>
                        <h2 className="text-lg sm:text-xl font-bold text-white">{title}</h2>
                        <p className="text-sm text-slate-400 mt-2">{description}</p>
                        <div className="text-3xl sm:text-4xl font-bold text-white mt-6">{value}</div>
                        <p className="text-sm text-indigo-300 font-semibold mt-2">{trend}</p>
                    </article>
                ))}
            </div>
            <article className="rounded-2xl sm:rounded-3xl bg-slate-900 p-4 sm:p-6 shadow-sm border border-slate-800">
                <h2 className="text-lg sm:text-xl font-bold text-white">Weekly performance</h2>
                <p className="text-sm text-slate-400 mt-1">Simple visual report block using mock values.</p>
                <div className="flex items-end gap-2 sm:gap-3 h-48 sm:h-64 mt-8">
                    {chartBars.map((height, index) => (
                        <div key={index} className="flex-1 rounded-t-xl sm:rounded-t-2xl bg-slate-800 relative overflow-hidden min-w-0">
                            <div className="absolute bottom-0 left-0 right-0 rounded-t-xl sm:rounded-t-2xl bg-indigo-500" style={{ height: `${height}%` }} />
                        </div>
                    ))}
                </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            {settingsCards.map(([title, description, status]) => (
                <article key={title} className="rounded-2xl sm:rounded-3xl bg-slate-900 p-4 sm:p-6 shadow-sm border border-slate-800">
                    ...
                    <h2 className="text-lg sm:text-xl font-bold text-white mt-5">{title}</h2>
                    <p className="text-sm text-slate-400 mt-2">{description}</p>
                </article>
            ))}
        </section>
...
function DemoApp() {
    ...
    if (!signedIn) {
        return (
            <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_#312e81,_#020617_50%,_#020617)] text-white flex items-center justify-center px-4 py-8 sm:px-6">
                <section className="w-full max-w-md rounded-2xl sm:rounded-3xl border border-white/10 bg-slate-900/90 p-5 sm:p-8 shadow-2xl backdrop-blur text-white">
                    <div className="flex items-center gap-3 mb-8">
                        <div className="h-11 w-11 sm:h-12 sm:w-12 rounded-2xl bg-indigo-500 flex items-center justify-center font-black text-lg sm:text-xl text-white shadow-lg shadow-indigo-950/40 shrink-0">N</div>
                        <div className="min-w-0">
                            <h1 className="text-xl sm:text-2xl font-bold text-white">Nexa Admin</h1>
                            <p className="text-xs sm:text-sm text-slate-200">Portfolio demo dashboard</p>
                        </div>
                    </div>
                    <div className="space-y-2 mb-6">
                        <h2 className="text-2xl sm:text-3xl font-bold text-white">Welcome back</h2>
                        <p className="text-sm sm:text-base text-slate-200 leading-relaxed">This public version runs in demo mode while the real API repository is being prepared.</p>
                    </div>
    ...
    return (
        <main className="min-h-screen bg-slate-950 text-slate-100">
            <div className="flex min-h-screen">
                <aside className="hidden lg:flex w-72 flex-col bg-slate-950 text-white p-6 shrink-0 border-r border-slate-800">
                ...
                <section className="flex-1 min-w-0 p-4 sm:p-5 md:p-8">
                    <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6 sm:mb-8">
                        <div className="min-w-0">
                            <p className="text-xs sm:text-sm font-semibold text-indigo-300">Portfolio dashboard</p>
                            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight text-white break-words">{activeSection === 'Overview' ? 'Nexa Admin Overview' : activeSection}</h1>
                            <p className="text-sm sm:text-base text-slate-400 mt-2 max-w-3xl">A responsive SaaS-style admin dashboard mockup ready for API integration.</p>
                        </div>
                        <button className="rounded-xl bg-white px-4 py-3 text-sm font-semibold text-slate-950 w-full sm:w-auto" onClick={() => setSignedIn(false)}>Sign out</button>
                    </header>
                    <div className="lg:hidden mb-6 -mx-4 px-4 overflow-x-auto">
                        <div className="flex gap-2 min-w-max pb-1">
                            {navigationItems.map((item) => (
                                <button key={item} type="button" onClick={() => setActiveSection(item)} className={`rounded-xl px-3 py-2 text-sm font-semibold transition whitespace-nowrap ${activeSection === item ? 'bg-indigo-500 text-white' : 'bg-slate-900 text-slate-300 shadow-sm'}`}>{item}</button>
                            ))}
                        </div>
                    </div>
