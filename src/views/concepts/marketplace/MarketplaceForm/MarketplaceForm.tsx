...
import type { MarketplaceFormSchema } from '../types'
import type { ZodType } from 'zod'
import type { CommonProps } from '@/@types/common'

type MarketplaceFormProps = {
    onFormSubmit: (values: MarketplaceFormSchema) => void
    defaultValues?: MarketplaceFormSchema
    newMarketplace?: boolean
} & CommonProps

const validationSchema: ZodType<MarketplaceFormSchema> = z.object({
    name: z.string().min(1, { message: 'Marketplace item name required!' }),
...
const MarketplaceForm = (props: MarketplaceFormProps) => {
    const {
        onFormSubmit,
        defaultValues = {
            imgList: [],
        ...
        children,
        // newMarketplace NO se usa aquí, pero lo recibe (y lo puedes usar si quieres condicionar algo)
    } = props
    ...
    const onSubmit = (values: MarketplaceFormSchema) => {
        onFormSubmit?.(values)
    }
