...
function isEmptyish(v: unknown) { return v == null || (typeof v === 'string' && v.trim() === '') }
function valueOrDash(v: unknown): string { return isEmptyish(v) ? '---------' : String(v) }
function pickNonEmpty(row: Rec | undefined, keys: string[]): unknown {
  if (!row) return undefined
  for (const k of keys) { const v = row[k]; if (!isEmptyish(v)) return v }
  return undefined
}
function pickDeepNonEmpty(obj: Rec | undefined, candidates: Array<string | string[]>): unknown {
  ...
  for (const c of candidates) {
    if (Array.isArray(c)) {
      let cur: unknown = obj; let ok = true
      for (const k of c) {
      ...
      const v = obj[c]; if (!isEmptyish(v)) return v
    }
  }
  return undefined
}
...
function fmtDateOrDash(raw: unknown): string {
  const s = toText(raw); if (!s) return '---------'
  const d = new Date(s); return isNaN(d.getTime()) ? '---------' : d.toISOString().slice(0,10)
}
function readList(raw: unknown): unknown[] {
  if (!raw) return []
  const candidates: unknown[] = [(raw as Rec)?.list,(raw as Rec)?.items,(raw as Rec)?.users,(raw as Rec)?.data,raw]
  for (const c of candidates) {
...
function readTotal(raw: unknown): number {
  ...
  const top = Number((raw as Rec).total ?? 0); if (top) return top
  const d = (raw as Rec).data; if (isRec(d)) return Number((d as Rec).total ?? 0)
  return 0
}
...
function NameCell({ row, usersMap }: { row: Rec; usersMap: UsersMap }) {
  const userId = String(pickNonEmpty(row, ['userId','user_id']) ?? (isRec(row.user) ? (row.user as Rec).id : '') ?? '')
  const user = usersMap.get(userId)
  const label =
    user?.name ||
    (isRec(row.user) && (toText((row.user as Rec).full_name) || toText((row.user as Rec).name) || toText((row.user as Rec).email))) ||
    (userId ? `ID ${userId}` : 'Sin nombre')
  return (
    <div className="flex items-center">
      <Link className="hover:text-primary ml-2 rtl:mr-2 font-semibold text-gray-900 dark:text-gray-100"
...
async function fetchUsersSubset(targetIds: string[]): Promise<UsersMap> {
  const want = new Set(targetIds.filter(Boolean)); const out: UsersMap = new Map()
  if (want.size === 0) return out
  const limit = 100; let pageIndex = 1
  for (;;) {
    const resp = await ApiService.fetchDataWithAxios<unknown>({ url: '/api/v1/users/', method: 'get', params: { pageIndex, pageSize: limit } })
    const list = readList(resp)
    for (const u of list) {
      if (!isRec(u)) continue
      const id = String(u.id ?? (u as Rec)._id ?? ''); if (!id || !want.has(id)) continue
      ...
      out.set(id, { name }); want.delete(id)
    }
...
async function fetchPropertiesSubset(targetIds: string[]): Promise<PropsMap> {
  const want = new Set(targetIds.filter(Boolean)); const out: PropsMap = new Map()
  if (want.size === 0) return out
  const limit = 100; let skip = 0
  for (;;) {
    const resp = await ApiService.fetchDataWithAxios<unknown>({ url: '/api/v1/communities/properties', method: 'get', params: { skip, limit } })
    const list = readList(resp)
    for (const p of list) {
      if (!isRec(p)) continue
      const id = String((p as Rec).id ?? (p as Rec).property_id ?? ''); if (!id || !want.has(id)) continue
      const number =
        toText((p as Rec).property_number) || toText((p as Rec).number) || toText((p as Rec).unit) || toText((p as Rec).code) || `#${id}`
      ...
      out.set(id, { number, floor: floor ?? '', block, createdAt }); want.delete(id)
    }
    if (want.size === 0) break
...
const ResidentsListTable = () => {
  ...
  const { userIds, propertyIds } = useMemo(() => {
    const u = new Set<string>(), p = new Set<string>()
    for (const r of residentsList as unknown as Rec[]) {
      const userId = String(pickNonEmpty(r, ['userId','user_id']) ?? ''); if (userId) u.add(userId)
      const pid = String(pickNonEmpty(r, ['propertyId','property_id']) ?? ''); if (pid) p.add(pid)
    }
  ...
  const columns: ColumnDef<Resident>[] = useMemo(() => [
    ...
    },
    {
      ...
      cell: (props) => {
        ...
        const role = toText(
          pickDeepNonEmpty(r, [
            'home_role','homeRole','role','role_name',
    ...
    },
    {
      ...
      cell: (props) => {
        ...
        const fromRow = pickDeepNonEmpty(r, [
          'startDate','start_date','start','assignment_start','created_at','createdAt',['assignment','start_date'],
        ])
  ...
  // clave de remonte como en Properties (🧩 añadimos communityId para garantizar remount al cambiar comunidad)
  const tableKey = useMemo(() => {
    const sKey = tableData?.sort?.key ?? ''
    const sOrd = tableData?.sort?.order ?? ''
    const q = tableData?.query ?? ''
    const pid = (filterData as Record<string, unknown>)?.['propertyId'] ?? ''
    const uid = (filterData as Record<string, unknown>)?.['userId'] ?? ''
    const own = (filterData as Record<string, unknown>)?.['isOwner'] ?? ''
    const sd = (filterData as Record<string, unknown>)?.['startDateFrom'] ?? ''
    const ed = (filterData as Record<string, unknown>)?.['endDateTo'] ?? ''
    const cid = String(communityId ?? '')
    return `${tableData.pageIndex}-${tableData.pageSize}-${sKey}-${sOrd}-${q}-${pid}-${uid}-${own}-${sd}-${ed}-${cid}`
  ...
  return (
    <DataTable
      ...
      loading={isLoading}
      /* Igual que en Properties: pasamos total/pageIndex/pageSize desde el backend/estado */
      pagingData={{
        total: Number(residentsListTotal) || 0,
