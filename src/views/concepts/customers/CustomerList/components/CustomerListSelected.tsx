// src/views/concepts/users/CustomerList/CustomerListSelected.tsx
...
type SelectedCustomer = Partial<Customer> & {
  avatar?: string
  img?: string
  name?: string
}
...
const CustomerListSelected = () => {
  ...
  const handleConfirmDelete = async () => {
    const ids = selectedCustomer
      .map((c: SelectedCustomer) => c.id)
  ...
  const getAvatarSrc = (c: SelectedCustomer) => c.avatar || c.img || ''
      ...
      >
        ...
        >
          {selectedCustomer.map((customer) => {
            const c = customer as SelectedCustomer
            return (
              <Tooltip key={String(c.id)} title={String(c.name ?? '')}>
                <Avatar size={30} src={getAvatarSrc(c)} alt="" />
              </Tooltip>
            )
          })}
        </Avatar.Group>
