...
const StatisticCard = ({ title, className, icon, value }: StatisticCardProps) => {
  return (
...
const ProjectOverview = ({ data }: ProjectOverviewProps) => {
  const ongoing = data?.ongoingProject ?? 0
  const completed = data?.projectCompleted ?? 0
  const upcoming = data?.upcomingProject ?? 0
    ...
    <Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 rounded-2xl mt-4">
        <StatisticCard
          ...
          className="bg-sky-100 dark:bg-sky-200/75"
          value={ongoing}
          icon={<TbProgressBolt />}
        />
        <StatisticCard
          title="Project completed"
          className="bg-emerald-100 dark:bg-emerald-200/75"
          value={completed}
          icon={<TbCopyCheck />}
        />
        <StatisticCard
          title="Upcoming project"
          className="bg-purple-100 dark:bg-purple-200/75"
          value={upcoming}
